<x-head title="Modifier Produit"></x-head>
...
<body class="bg-gray-100">
    @if (Session::has('user_type') == 'admin')
        ...
        <div class="container mx-auto p-6 bg-white shadow-md rounded-lg mt-6">
            <h1 class="text-3xl font-bold mb-6 text-gray-800">Modifier un Produit</h1>
            ...
            <form action="{{ route('produits.update', $produit->id) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nom" id="nom" value="{{ old('nom', $produit->nom) }}"
                        class="mt-1 block w-full rounded-md shadow-sm @error('nom') border-red-500 @enderror" required>
                    @error('nom')
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="prix" id="prix" value="{{ old('prix', $produit->prix) }}"
                        class="mt-1 block w-full  rounded-md shadow-sm @error('prix') border-red-500 @enderror"
                        required>
                    @error('prix')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-4">
                    <label for="image" class="block text-gray-700 font-medium">Image du Produit (laisser vide si vous
                        ne souhaitez pas changer)</label>
                ...
                <div>
                    <button type="submit"
                        class="px-4 py-2 bg-blue-600 text-white font-medium rounded-md hover:bg-blue-700">Mettre à
                        Jour</button>
                </div>
            </form>
        </div>
