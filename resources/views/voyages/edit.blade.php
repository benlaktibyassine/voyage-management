<x-head title="Modifier le Voyage"></x-head>
    ...
    @if (Session::has('user_type') == 'admin')
        ...
        <div class="container mx-auto p-6">
            <div class="bg-white shadow-md rounded-lg p-6 max-w-md mx-auto">
                <h1 class="text-3xl font-bold mb-6 text-gray-800">Modifier le Voyage</h1>
                ...
                <form action="{{ route('voyages.update', $voyage->mat) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-4">
                        ...
                        <select name="camion_id" id="camion_id"
                            ...
                            @foreach ($camions as $camion)
                                <option value="{{ $camion->id }}"
                                    {{ $voyage->camion_id == $camion->id ? 'selected' : '' }}>
                                    {{ $camion->matricule }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-4 text-center">
                        <button type="submit"
                            class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:outline-none focus:border-blue-700 focus:ring ring-gray-300 active:bg-blue-700 disabled:opacity-25 transition ease-in-out duration-150">Mettre
                            à jour</button>
                    </div>
                </form>
            </div>
